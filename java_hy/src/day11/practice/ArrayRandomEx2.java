...
public class ArrayRandomEx2 {
	...
	public static void main(String[] args) {
		...
		while(count < 3) {
			int random = (int)(Math.random()*(max - min + 1) + min);
			//이미 저장된 랜덤수들과 하나한 비교해서 같은게 있는지 확인하여 있으면 반복문 종료
			for(i = 0; i < count; i++) {
				if(random == arr[i]) {
					break;
				}
			}
			/*위 반복문에서 break를 만나지 않으면 i는 count 일때
			 * 종료하기 때문에 반복문 종료 후 i는 count와 같다.
			 * 하지만 break를 만나서 중간에 빠져 나오면 i는 count 보다 작다. */
			if(i == count) {
				arr[count] = random;
				count++;
				//arr[count++] = random; //위 코드와 같은 결과
			}
		}
			System.out.println("==========");
			printArray(arr);
	...
	/** min에서 max사이의 랜덤한 수를 생셩해서 배열에 저장하는 메서드
