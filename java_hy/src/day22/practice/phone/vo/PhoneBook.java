...
public class PhoneBook {
	...
	public boolean updateName(String name, String updateName) {
		...
		//위에 두줄을 한줄로 쓰면 : list.get(index).setName(updateName);
		return true;
		
		/* 기본 자료형은 = 을 이용하면 값을 복사하기 때문에
		 * 한쪽의 값이 바뀌어도 다른쪽은 안 바뀜 
		 * int num1 = 1;		//num1 = 1
		 * int num2 = num1; 	//num1 = 1, num2 = 1
		 * num2 = 10;			//num1 = 1, num2 = 10
		 //참조변수는 =을 이용하면 주소값을 복사하기 때문에 
		 //한쪽의 값이 바뀌면 다른쪽도 바뀜
		 //예를 들면 pn1에 저장된 주소가 0x1000
		 * PhoneNumber pn = new PhoneNumber("a", "010-1234-5678")
	...
	public boolean updateNumber(String name, String number) {
		...
		//전화번호 패턴이 맞으면 전화번호 객체를 생성 했을 대 전화번호가 있고,
		//패턴이 맞지않으면 빈 문자열이 있다.
