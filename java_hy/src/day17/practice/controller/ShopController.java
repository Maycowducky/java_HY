...
public class ShopController implements RunInterface{
	...
	private Customer customerList[] = new Customer[10];//최대 10명 고객관리
	...
	private void runMenu(int menu) {
		...
		case 1:	
			...
			//제품 리스트와 고객 리스트와 판매내역 정보
			int salesCount = shopService.sell(list, count, 
					customerList, customerCount, salesHistory, this.salesCount, name, amount, phoneNumber);
			if(salesCount == -1) {
			...
			break;
		case 2:
			//입고할 제품 정보를 입력 받아 제품 객체로 생성
			Product product = inputStoreProduct();
			//제품 리스트와 제품 수를 주고, 입고된 제품을 주고, 제품 입고를 관리하라고 시킴
			//제품과 관련된 정보를 자체에서 입력
			//리턴값을 count로 저장하는 이유는 : 새 제품이 입고된 경우 제품 리스트에 추가 되고 제품수가 1 증가해야하기 때문에
	...
	private Customer inputRegisterCustomer() {
		...
		/* Customer customer = new Customer (name, phoneNumer);
		 * return customer; 위에 한줄이 두줄 코드랑 같음
		 */
	}
