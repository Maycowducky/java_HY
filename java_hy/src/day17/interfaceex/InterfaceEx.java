...
public class InterfaceEx {
	...
	public static void main(String[] args) {
		...
		runInterface.run();
		//RunInterface에는 load가 추상메서드를 존재하지 않기 때문에 에러 발생
		//runInterface.load();
		//아래 코드는 Run클래스의 객체로 클래스변환이 됐기 때문에 load() 메서드를 호출할 수 있음
		((Run)runInterface).load();
		run(new ShopManager());// 판매관리 프로그램을 실행
		//run(new VocabularyNoteController()); //단어장 프로그램을 실행
	}
	/*인터페이스 매개변수 다형성
	 ...
	 * 와서 프로그램을 실행할 수 있다 
	 */
	public static void run(/*프로그램 객체*/RunInterface program) {
		//프로그램 실행
		program.run();
	}
}
...
//Run에 마우스 오버 해서 Add unimplemented Method 클릭하면 자동으로 오버라이딩이 됨
