...
-- SELECT * FROM B JOIN A ON A.A = B.b;
SELECT 
	OR_ME_ID AS 주문아이디,
    PR_NAME AS 제품명,
    OP_NAME AS 옵션명,
...
-- KH가 제품명에 포함된 제품들중에서 1페이지에 해당하는 제품들을 검색하는 쿼리 (한페이지에는 제품이 최대 5개)
SELECT 
    *
FROM
    PRODUCT
WHERE
    PR_NAME LIKE CONCAT('%', 'KH', '%')
LIMIT 0, 5;
-- KH가 제품명에 포함된 제품들중에서 2페이지에 해당하는 제품들을 검색하는 쿼리 (한페이지에는 제품이 최대 5개)
SELECT 
    *
FROM
    PRODUCT
...
-- ABC123 회원이 작성한 리뷰드가 있는 제품명을 조회
SELECT 
    PR_NAME AS 리뷰제품
FROM
    REVIEW
...
-- 제품 옵션별 판매량(반품, 한불을 제외한)을 조회하는 쿼리 (제품명, 옵션명, 판매량)
SELECT 
	PR_NAME AS 제품명,
    OP_NAME AS 옵션명,
    IFNULL(SUM(OL_AMOUNT),0) AS 판매량
...
WHERE
	OR_STATE IS NULL OR OR_STATE NOT IN ('반품', '환불')
GROUP BY OP_NUM;
-- 제품 옵션별 판매량(반품, 한불을 제외한)을 조회하는 쿼리 (제품명, 옵션명, 판매량, 총판매금액)
SELECT 
	PR_NAME AS 제품명,
    OP_NAME AS 옵션명,
    IFNULL(SUM(OL_AMOUNT),0) AS 판매량,
