/*
join 문법
-- 테이블 1의 외래키가 테이블2의 기본키를 참조하는 경우
-- JOIN은 일반적으로 외래키아 기본키를 연결하지만, 상황에 따라서는 외래키가 아닌 속성을 기본키와 연결할 수 있다
SELECT 
	테이블1. 속성명1, 테이블1, 속성명2, ..., 테이블2. 속성명1, 테이블2. 속성명2, ...
FROM
	테이블1
JOIN -- INNER 조인과 OUTER 조인 둘다 가능 INNER JOIN | LEFT JOIN | RIGHT JOIN -- INNER 조인과 OUTER 조인 둘다 가능
	테이블2
ON
	테이블1.외래키 = 테이블2.외래키
...
SELECT NAME AS 유재석학번, COUNT(*) AS 수강과목수
FROM
	COURSE
JOIN
	STUDENT
...
-- 유재석 학생이 수강하는 과목명을 조회하는 쿼리ALTER
SELECT 
