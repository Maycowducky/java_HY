...
<script type="text/javascript">
   ...
   function getCommentList(cri){
      ajaxJsonToJson(false,'post','/comment/list/${board.bo_num}', cri ,(data)=>{
    	  ...
    	  createCommentList(data.list, '.comment-pagination');
      });
   }
   ...
   //댓글 리스트가 주어지면 댓글을 만들어서 target 안에 넣어주는 함수
   function createCommentList(commentList, target){
	   let str = '';
       if(commentList.list.length == 0){
      	 str = '<div class="border rounded-sm border-primary p-3 mt-3">등록된 댓글이 없습니다.</div>';
       }
       for(comment of commentList.list){
    	  let btnStr = '';
          ...
          str += `
             <div class="border rounded-sm border-primary p-3 mt-3">
